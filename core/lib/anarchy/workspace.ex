defmodule Anarchy.Workspace do
  @moduledoc """
  Creates isolated per-task workspaces for parallel Codex agents.
  """
  ...
  defp issue_context(%{id: task_id, title: title}) when is_binary(task_id) do
    short_id = String.slice(task_id, 0, 8)
    label = if title, do: "#{short_id}_#{title}", else: short_id
    %{
      issue_id: task_id,
  ...
  defp issue_log_context(%{issue_id: issue_id, issue_identifier: issue_identifier}) do
    "task_id=#{issue_id || "n/a"} task=#{issue_identifier || "unknown"}"
  end
end
