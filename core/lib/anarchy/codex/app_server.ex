defmodule Anarchy.Codex.AppServer do
  @moduledoc """
  ...
  require Logger
  alias Anarchy.{Config, PathSafety}
  ...
  def run_turn(
    ...
    tool_executor =
      Keyword.get(opts, :tool_executor, fn tool, _arguments ->
        %{"success" => false, "contentItems" => [%{"type" => "inputText", "text" => "Unsupported dynamic tool: #{inspect(tool)}"}]}
      end)
  ...
  @impl Anarchy.Runtime.AgentProtocol
  def resume_session(_session_id) do
    {:error, :not_supported}
  end
  ...
  defp session_holder(session, opts) do
    receive do
      {:send_prompt, prompt} ->
        issue = Map.get(opts, :issue, %{identifier: "unknown", title: "unknown", id: "unknown"})
        run_turn(session, prompt, issue, Keyword.new(Map.get(opts, :turn_opts, %{})))
        session_holder(session, opts)
  ...
  defp send_initialize(port) do
    payload = %{
      ...
      "params" => %{
        ...
        "clientInfo" => %{
          "name" => "anarchy-orchestrator",
          "title" => "Anarchy Orchestrator",
          "version" => "0.1.0"
        }
      }
    }
  ...
  defp start_turn(port, thread_id, prompt, issue, workspace, approval_policy, turn_sandbox_policy) do
    send_rpc_message(port, %{
      ...
      "params" => %{
        ...
        "cwd" => Path.expand(workspace),
        "title" => "#{Map.get(issue, :identifier, issue[:id] || issue.id)}: #{issue.title}",
        "approvalPolicy" => approval_policy,
  ...
  defp issue_context(%{id: task_id, identifier: identifier}) do
