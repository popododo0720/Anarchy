defmodule Anarchy.Config.Schema do
  @moduledoc false
  ...
  defmodule Tracker do
    ...
    embedded_schema do
      field(:kind, :string, default: "postgres")
    ...
    def changeset(schema, attrs) do
      ...
      |> cast(
        attrs,
        [:kind, :database_url, :active_states, :terminal_states],
        empty_values: []
      )
    end
  end
  ...
  defmodule Workspace do
    ...
    @primary_key false
    embedded_schema do
      field(:root, :string, default: Path.join(System.tmp_dir!(), "anarchy_workspaces"))
    end
  ...
  @spec normalize_task_state(String.t()) :: String.t()
  def normalize_task_state(state_name) when is_binary(state_name) do
  ...
  def normalize_state_limits(limits) when is_map(limits) do
    Enum.reduce(limits, %{}, fn {state_name, limit}, acc ->
      Map.put(acc, normalize_task_state(to_string(state_name)), limit)
    end)
  end
  ...
  defp finalize_settings(settings) do
    tracker = %{
      settings.tracker
      | database_url: resolve_secret_setting(settings.tracker.database_url, System.get_env("DATABASE_URL"))
    }

    workspace = %{
      settings.workspace
      | root: resolve_path_value(settings.workspace.root, Path.join(System.tmp_dir!(), "anarchy_workspaces"))
    }
  ...
  defp default_turn_sandbox_policy(workspace) do
    writable_root =
      if is_binary(workspace) and workspace != "" do
        ...
        Path.expand(Path.join(System.tmp_dir!(), "anarchy_workspaces"))
      end
