defmodule Anarchy.SpecsCheck do
  @moduledoc false
  ...
  defp collect_elixir_files(path) do
    # Normalize backslashes to forward slashes for Path.wildcard compatibility on Windows
    normalized_path = String.replace(path, "\\", "/")

    cond do
      File.regular?(normalized_path) and String.ends_with?(normalized_path, ".ex") ->
        [normalized_path]

      File.dir?(normalized_path) ->
        Path.wildcard(Path.join(normalized_path, "**/*.ex"))
