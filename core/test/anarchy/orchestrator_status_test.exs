defmodule Anarchy.OrchestratorStatusTest do
  ...
  test "orchestrator snapshot reflects last codex update and session id" do
    ...
    assert snapshot_entry.task_id == issue_id
  ...
  test "status dashboard renders no project link when no dashboard url" do
    snapshot_data =
      {:ok,
       %{
         running: [],
  ...
  test "status dashboard renders dashboard url on its own line when server port is configured" do
    previous_port_override = Application.get_env(:anarchy, :server_port_override)

    on_exit(fn ->
      if is_nil(previous_port_override) do
        Application.delete_env(:anarchy, :server_port_override)
      else
        Application.put_env(:anarchy, :server_port_override, previous_port_override)
      end
    end)

    Application.put_env(:anarchy, :server_port_override, 4000)
  ...
  test "application configures a rotating file logger handler" do
    assert {:ok, handler_config} = :logger.get_handler_config(:anarchy_disk_log)
  ...
  test "status dashboard enriches wrapper reasoning and message streaming events with payload context" do
    reasoning_message = %{
      ...
      message: %{
        ...
        "params" => %{
          "msg" => %{
            "payload" => %{"summaryText" => "compare retry paths for task polling"}
          }
        }
      }
    }
    ...
    assert StatusDashboard.humanize_codex_message(reasoning_message) =~
             "reasoning update: compare retry paths for task polling"
