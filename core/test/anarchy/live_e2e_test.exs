defmodule Anarchy.LiveE2ETest do
  use Anarchy.TestSupport

  require Logger

  # Stub for removed Linear client - this test is always skipped
  defmodule Client do
    def graphql(_query, _variables), do: {:error, :linear_removed}
  end
  ...
  @live_e2e_skip_reason "Linear integration has been removed; this test is disabled"

  @team_query """
  query AnarchyLiveE2ETeam($key: String!) {
  ...
  mutation AnarchyLiveE2ECreateProject($name: String!, $teamIds: [String!]!) {
  ...
  mutation AnarchyLiveE2ECreateIssue(
    $teamId: String!
    $projectId: String!
    $title: String!
    $description: String!
  ...
  query AnarchyLiveE2EProjectStatuses {
  ...
  @issue_details_query """
  query AnarchyLiveE2EIssueDetails($id: String!) {
    issue(id: $id) {
      id
      identifier
      state {
  ...
  mutation AnarchyLiveE2ECompleteProject($id: String!, $statusId: String!, $completedAt: DateTime!) {
  ...
  test "creates a real Linear project and issue, then runs a real Codex turn" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "anarchy-live-e2e-#{System.unique_integer([:positive])}"
      )

    workflow_root = Path.join(test_root, "workflow")
    workflow_file = Path.join(workflow_root, "WORKFLOW.md")
    workspace_root = Path.join(test_root, "workspaces")
    team_key = System.get_env("ANARCHY_LIVE_E2E_TEAM_KEY") || @default_team_key
    codex_command = System.get_env("ANARCHY_LIVE_CODEX_COMMAND") || "codex app-server"
    ...
    try do
      ...
      issue =
        create_issue!(
          ...
          project["id"],
          active_state["id"],
          "Anarchy live e2e issue for #{project["name"]}"
        )
  ...
  defp live_prompt(project_slug) do
    """
    You are running a real Anarchy end-to-end test.
