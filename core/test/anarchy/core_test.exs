defmodule Anarchy.CoreTest do
  ...
  # Skipped: this test validated the on-disk WORKFLOW.md with Linear config which has been removed.
  # test "current WORKFLOW.md file is valid and complete" do
  # end

  # Skipped: Linear API token and assignee config fields have been removed.
  ...
  test "workflow load accepts unterminated front matter with an empty prompt" do
    ...
    assert {:ok, %{config: %{"tracker" => %{"kind" => "postgres"}}, prompt: "", prompt_template: ""}} =
             Workflow.load(workflow_path)
  end
  ...
  test "Anarchy.start_link delegates to the orchestrator" do
    write_workflow_file!(Workflow.workflow_file_path(), tracker_kind: "memory")
    Application.put_env(:anarchy, :memory_tracker_issues, [])
    orchestrator_pid = Process.whereis(Anarchy.Orchestrator)

    on_exit(fn ->
      if is_nil(Process.whereis(Anarchy.Orchestrator)) do
        case Supervisor.restart_child(Anarchy.Supervisor, Anarchy.Orchestrator) do
          {:ok, _pid} -> :ok
    ...
    if is_pid(orchestrator_pid) do
      assert :ok = Supervisor.terminate_child(Anarchy.Supervisor, Anarchy.Orchestrator)
    end

    assert {:ok, pid} = Anarchy.start_link()
    assert Process.whereis(Anarchy.Orchestrator) == pid

    GenServer.stop(pid)
  end

  # Skipped: Linear client has been removed; state reconciliation uses Tracker adapter now.
  ...
  test "non-active issue state stops running agent without cleaning workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "anarchy-nonactive-reconcile-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      updated_state = Orchestrator.reconcile_task_states_for_test(%{issue_id => :Backlog}, state)
  ...
  test "terminal issue state stops running agent and cleans workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "anarchy-terminal-reconcile-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      updated_state = Orchestrator.reconcile_task_states_for_test(%{issue_id => :Closed}, state)
  ...
  test "missing running issues stop active agents without cleaning the workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "anarchy-missing-running-reconcile-#{System.unique_integer([:positive])}"
      )

    previous_memory_issues = Application.get_env(:anarchy, :memory_tracker_issues)
    ...
    try do
      ...
      assert {:ok, workspace} =
               Anarchy.PathSafety.canonicalize(Path.join(test_root, issue_identifier))
  ...
  test "reconcile updates running issue state for active issues" do
    ...
    state = %Orchestrator.State{
      running: %{
        issue_id => %{
          ...
          ref: nil,
          title: "MT-557",
          task: %{id: issue_id, identifier: "MT-557", status: :Todo},
          codex_app_server_pid: nil,
          started_at: DateTime.utc_now()
        }
      },
    ...
    updated_state = Orchestrator.reconcile_task_states_for_test(%{issue_id => :"In Progress"}, state)
    updated_entry = updated_state.running[issue_id]

    assert Map.has_key?(updated_state.running, issue_id)
    assert MapSet.member?(updated_state.claimed, issue_id)
    assert updated_entry.task.status == :"In Progress"
  end

  test "reconcile stops running task when it moves to a non-active non-terminal state" do
    ...
    state = %Orchestrator.State{
      running: %{
        issue_id => %{
          ...
          ref: nil,
          title: "MT-561",
          task: %{id: issue_id, identifier: "MT-561", status: :"In Progress"},
          codex_app_server_pid: nil,
          started_at: DateTime.utc_now()
        }
      },
  ...
  test "normal worker exit schedules active-state continuation retry" do
    ...
    running_entry = %{
      ...
      ref: ref,
      title: "MT-558",
      task: %{id: issue_id, identifier: "MT-558", status: :"In Progress"},
      codex_app_server_pid: nil,
      started_at: DateTime.utc_now()
    }
  ...
  test "abnormal worker exit increments retry attempt progressively" do
    ...
    running_entry = %{
      ...
      title: "MT-559",
      retry_attempt: 2,
      task: %{id: issue_id, identifier: "MT-559", status: :"In Progress"},
      codex_app_server_pid: nil,
      started_at: DateTime.utc_now()
    }
    ...
    assert %{attempt: 3, due_at_ms: due_at_ms, title: "MT-559", error: "agent exited: :boom"} =
  ...
  test "first abnormal worker exit waits before retrying" do
    ...
    running_entry = %{
      ...
      ref: ref,
      title: "MT-560",
      task: %{id: issue_id, identifier: "MT-560", status: :"In Progress"},
      codex_app_server_pid: nil,
      started_at: DateTime.utc_now()
    }
    ...
    assert %{attempt: 1, due_at_ms: due_at_ms, title: "MT-560", error: "agent exited: :boom"} =
             state.retry_attempts[issue_id]
  ...
  defp restore_app_env(key, nil), do: Application.delete_env(:anarchy, key)
  defp restore_app_env(key, value), do: Application.put_env(:anarchy, key, value)

  # Skipped: Linear client has been removed; uses Tracker adapter now.
  ...
  test "prompt builder uses a sensible default template when workflow prompt is blank" do
    ...
    assert prompt =~ "You are working on a task."
    assert prompt =~ "ID: MT-777"
    assert prompt =~ "Title: Make fallback prompt useful"
    assert prompt =~ "Body:"
    assert prompt =~ "Include enough issue context to start working."
    assert Config.workflow_prompt() =~ "{{ task.id }}"
  ...
  test "prompt builder reports workflow load failures separately from template parse errors" do
    original_workflow_path = Workflow.workflow_file_path()
    workflow_store_pid = Process.whereis(Anarchy.WorkflowStore)

    on_exit(fn ->
      Workflow.set_workflow_file_path(original_workflow_path)

      if is_pid(workflow_store_pid) and is_nil(Process.whereis(Anarchy.WorkflowStore)) do
        Supervisor.restart_child(Anarchy.Supervisor, Anarchy.WorkflowStore)
      end
    end)

    assert :ok = Supervisor.terminate_child(Anarchy.Supervisor, Anarchy.WorkflowStore)
  ...
  # Skipped: This test validated the on-disk WORKFLOW.md with Linear-specific template which has been removed.
  ...
  # Windows: shell scripts not supported
  @tag :skip
  test "agent runner keeps workspace after successful codex run" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "anarchy-agent-runner-retain-workspace-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner forwards timestamped codex updates to recipient" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "anarchy-agent-runner-updates-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner continues with a follow-up turn while the issue remains active" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "anarchy-agent-runner-continuation-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner stops continuing once agent.max_turns is reached" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "anarchy-agent-runner-max-turns-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server starts with workspace cwd and expected startup command" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "anarchy-app-server-args-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server startup command supports codex args override from workflow config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "anarchy-app-server-custom-args-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server startup payload uses configurable approval and sandbox settings from workflow config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "anarchy-app-server-policy-overrides-#{System.unique_integer([:positive])}"
      )
