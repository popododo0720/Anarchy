defmodule Mix.Tasks.Workspace.BeforeRemoveTest do
  ...
  # Windows: shell scripts not supported (with_path manipulates PATH for sh binaries)
  @tag :skip
  ...
  # Windows: shell scripts not supported (with_fake_gh_and_git writes #!/bin/sh binaries)
  @tag :skip
  test "uses current branch for lookup when branch option is omitted" do
    with_fake_gh_and_git(
      ...
      fn log_path ->
        ...
        assert log =~
                 "pr list --repo org/anarchy --head feature/workpad --state open --json number --jq .[].number"

        assert log =~ "pr close 101 --repo org/anarchy"
        assert log =~ "pr close 102 --repo org/anarchy"
      end
    )
  end

  # Windows: shell scripts not supported (with_fake_gh writes #!/bin/sh binaries)
  @tag :skip
  test "closes open pull requests for the branch and tolerates close failures" do
    with_fake_gh(fn log_path ->
      ...
      assert log =~ "pr list --repo org/anarchy --head feature/workpad --state open --json number --jq .[].number"
      assert log =~ "pr close 101 --repo org/anarchy"
      assert log =~ "pr close 102 --repo org/anarchy"
  ...
  test "formats close failures without command stderr output" do
    with_fake_gh(
      ...
      fn log_path ->
        ...
        log = File.read!(log_path)
        assert log =~ "pr list --repo org/anarchy --head feature/no-output --state open --json number --jq .[].number"
        assert log =~ "pr close 102 --repo org/anarchy"
      end
    )
  end
  ...
  test "no-ops when PR list fails for current branch" do
    with_fake_gh(
      ...
      fn log_path ->
        ...
        assert log =~
                 "pr list --repo org/anarchy --head feature/list-fails --state open --json number --jq .[].number"
