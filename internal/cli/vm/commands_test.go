...
func TestRunListPrintsReadableSummary(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`[{"name":"vm1","phase":"Running","image":"ubuntu-24.04","network":"tenant-a","subnetRef":"tenant-a","privateIp":"10.0.0.10","networkAttachments":[{"name":"nic0","network":"tenant-a","subnetRef":"tenant-a","role":"external","ipAddress":"10.0.0.10","primary":true}]}]`))
	}))
	defer server.Close()
	...
	for _, want := range []string{"Name: vm1", "Phase: Running", "Network: tenant-a", "Subnet: tenant-a", "Private IP: 10.0.0.10", "Attachments: nic0[external](tenant-a/tenant-a, nad=unknown, ip=10.0.0.10, primary=true)"} {
...
func TestRunShowPrintsReadableDetail(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"name":"vm1","phase":"Running","image":"ubuntu-24.04","cpu":2,"memory":"4Gi","network":"tenant-a","subnetRef":"tenant-a","privateIp":"10.0.0.10","networkAttachments":[{"name":"nic0","network":"tenant-a","subnetRef":"tenant-a","role":"external","ipAddress":"10.0.0.10","primary":true}]}`))
	}))
	defer server.Close()
	...
	for _, want := range []string{"Name: vm1", "Image: ubuntu-24.04", "Network: tenant-a", "Subnet: tenant-a", "Attachments: nic0[external](tenant-a/tenant-a, nad=unknown, ip=10.0.0.10, primary=true)"} {
		if !bytes.Contains(out.Bytes(), []byte(want)) {
			t.Fatalf("output = %q, want %q", out.String(), want)
		}
	}
...
func TestRunCreateAcceptsExplicitAttachmentsJSON(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for _, want := range []string{
			"\"networkAttachments\":[{\"name\":\"nic0\",\"network\":\"default\",\"subnetRef\":\"tenant-a\",\"role\":\"external\",\"primary\":true},{\"name\":\"nic1\",\"network\":\"default\",\"subnetRef\":\"tenant-b\",\"nadRef\":\"anarchy-system/tenant-b-net\",\"role\":\"internal\",\"primary\":false}]",
			"\"subnetRef\":\"tenant-a\"",
		} {
			if !bytes.Contains(buf.Bytes(), []byte(want)) {
				t.Fatalf("body = %s, want %s", buf.String(), want)
	...
	attachments := `[{"name":"nic0","network":"default","subnetRef":"tenant-a","role":"external","primary":true},{"name":"nic1","network":"default","subnetRef":"tenant-b","nadRef":"anarchy-system/tenant-b-net","role":"internal","primary":false}]`
	var out bytes.Buffer
