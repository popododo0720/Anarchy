...
func TestRunPublicIPPrintsReport(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"name":"fip-01","status":"pending","reason":"ovnfip_missing","code":"public_ip_not_realized","findings":["floating ip rule not realized yet"]}`))
	}))
	defer server.Close()
	...
	for _, want := range []string{"Name: fip-01", "Status: pending", "Reason: ovnfip_missing", "Code: public_ip_not_realized", "- floating ip rule not realized yet"} {
